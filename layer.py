...
from vllm.moe_logger import get_moe_logger 
...
def _infer_layer_index_from_prefix(prefix: str) -> int:
    """
    Best-effort guess of layer index from the module prefix.

    Example prefix (Qwen MoE): 'model.layers.0.mlp.experts'
    We look for 'layers.<int>'.
    """
    if not prefix:
        return -1
    parts = prefix.split(".")
...
class FusedMoE(CustomOp):
    ...
    def __init__(
        ...
        # === MoE logging: track layer index and logger ===
        # Store the prefix, e.g. 'model.layers.0.mlp.experts'
        self.prefix = prefix
        # Infer which transformer layer this MoE belongs to
        self.layer_index = _infer_layer_index_from_prefix(prefix)
        # Create (or get) the global MoE logger singleton, if enabled
        self.moe_logger = get_moe_logger(top_k)
        # === end MoE logging init ===
    ...
    def select_experts(
        ...
        # === MoE logging hook (no-op when logger is disabled) ===
        if self.moe_logger is not None:
            # Only log the single layer configured by VLLM_MOE_LAYER
            layer_to_log = int(os.getenv("VLLM_MOE_LAYER", "0"))
            if self.layer_index == layer_to_log:
                # For this assignment we use a simple request id
                self.moe_logger.log_routes(
                    layer=self.layer_index,
                    topk_ids=topk_ids,
                    topk_weights=topk_weights,
                    req_id="r1",
                )
        # === end MoE logging hook ===
